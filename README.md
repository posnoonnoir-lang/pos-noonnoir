...
| **ORM** | Prisma v7 (28 models, schema synced) | ✅ |
...
| **Toast** | Sonner | ✅ |
| **Data** | Server Actions (mock data, Prisma queries planned) | ✅ |
...
# 2. Run (mock data, no DB needed)
npx next dev --port 3001
```

Open [http://localhost:3001](http://localhost:3001) → Login PIN: **1234** (Owner)

> **Note:** Hiện tại chạy với mock data (in-memory). Supabase + Prisma sẽ được kết nối sau.
...
pos-noonnoir/
├── prisma/
│   ├── schema.prisma          # 28 models, full schema (synced)
│   ├── seed.ts                # Database seeder
│   └── reset.ts               # Database reset utility
├── src/
│   ├── actions/               # Server Actions (mock data)
...
│   ├── lib/
│   │   ├── mock-data.ts       # Mock products, tables, zones
...
|--------|----------------|:--------:|
| **Real DB Queries** | Connect mock → real Prisma queries | P1 |
...
| **Status** | **🚀 V2 Features Complete** — 25 routes, 33 modules, 27 server action files, 28 Prisma models, Vercel deployed |
...
*Last updated: March 11, 2026 — V2 Features Complete v5.0*
